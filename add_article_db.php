<?php
	...
	function add_to_articles($db) {
		...
		try
		{
			...
			$sql_query = "INSERT INTO articles(title,publicate_date,update_date,nb_views, discipline, article_owner, content)
						  VALUES (?,?,?,?,?,?,?)";
			$array = array($title,$publicate_date,$update_date,$nb_views,$discipline,$article_owner, $_POST['description']);
			$query = $db->prepare($sql_query);
			$query->execute($array);
